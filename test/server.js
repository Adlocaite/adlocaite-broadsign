...
// Ensure version.js exists for the package — build.sh normally generates it,
// but test:serve should also work on a fresh checkout without a prior build.
const versionFile = path.join(__dirname, '..', 'package', 'js', 'version.js');
if (!fs.existsSync(versionFile)) {
  const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'), 'utf8'));
  fs.writeFileSync(versionFile,
    `// AUTO-GENERATED by test/server.js fallback — DO NOT EDIT\n` +
    `// Normally regenerated by build.sh from package.json on every build.\n` +
    `window.ADLOCAITE_VERSION = '${pkg.version}';\n`
  );
  console.log(`   📝 Generated package/js/version.js (version ${pkg.version})`);
}
